...
impl<T: Emit> Items<T> {
    ...
    pub(super) fn emit_sep(&self, doc: &mut Doc, sep: &Elem) {
        ...
        while let Some(item) = iter.next() {
            ...
            // A comment item ending in a `/* lang */` annotation fuses the
            // annotation onto the following item.
            if let Item::Comments(trivia) = item
                && let Some((ann @ TriviaPiece::LanguageAnnotation(_), preceding)) =
                    trivia.split_last()
                && let Some(Item::Item(next)) = iter.peek()
            {
                let mut pre = Trivia::new();
                pre.extend(preceding.iter().cloned());
                pre.emit(doc);
                ann.emit(doc);
